"""
Homoglyph Checker — Streamlit Version
...
# Hanya karakter Cyrillic yang menyamar sebagai Latin
CYRILLIC_HOMOGLYPHS = set(LATIN_TO_CYRILLIC.values())
...
def detect_characters(text):
    ...
    for char in text:
        ...
        # FIX: hanya tandai sebagai homoglyph jika karakternya adalah Cyrillic
        # yang tampak seperti Latin — bukan huruf Latin asli
        is_homoglyph = char in CYRILLIC_HOMOGLYPHS
...
# ── Mode: Deteksi Karakter ──
    ...
    if teks:
        ...
        if rows:
            ...
            if homoglyphs:
                st.warning(f"⚠️ Ditemukan **{len(homoglyphs)}** karakter homoglyph Cyrillic!")
            else:
                st.success("✅ Tidak ada homoglyph terdeteksi. Semua karakter aman.")
...
# FIX: hapus emoji dari label expander agar tidak saling timpa
with st.expander("Lihat semua mapping karakter Latin ke Cyrillic"):
    mapping_data = [
        {
            "Latin": k,
            "Cyrillic (homoglyph)": v,
            "Unicode Latin": f"U+{ord(k):04X}",
            "Unicode Cyrillic": f"U+{ord(v):04X}",
        }
